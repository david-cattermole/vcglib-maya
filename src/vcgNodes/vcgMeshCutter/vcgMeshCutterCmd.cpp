...
vcgMeshCutter::vcgMeshCutter()
//
//  Description:
//    meshCutter constructor
//
{
//  m_percent = 100.0;
//  m_maxTime = 3600.0;
...
//
//  Description:
//    meshCutter destructor
//
{}

void *vcgMeshCutter::creator()
//
//  Description:
...
//
{
  return new vcgMeshCutter();
}

bool vcgMeshCutter::isUndoable() const
//
//  Description:
...
MStatus vcgMeshCutter::doIt(const MArgList &args)
//
//  Description:
//    implements the MEL meshCutter command.
//
//  Arguments:
...
//
{
  ...
  setModifierNodeType(vcgMeshCutterNode::id);
  ...
  if (found)
  {
    ...
    {
      setResult("meshCutter command succeeded!");
    }
    else
    {
      displayError("meshCutter command failed!");
    }
  }
  else
  {
    displayError("meshCutter command failed: Unable to find selected components");
    status = MS::kFailure;
  }
...
MStatus vcgMeshCutter::redoIt()
//
//  Description:
//    Implements redo for the MEL meshCutter command.
//
...
MStatus vcgMeshCutter::undoIt()
//
//  Description:
//    implements undo for the MEL meshCutter command.
//
...
//
{
  ...
  if (status == MS::kSuccess)
  {
    setResult("meshCutter undo succeeded!");
  }
  else
  {
    setResult("meshCutter undo failed!");
  }

  return status;
}

MStatus vcgMeshCutter::initModifierNode(MObject modifierNode)
{
  MStatus status;
...
MStatus vcgMeshCutter::directModifier(MObject mesh)
{
  MStatus status;
