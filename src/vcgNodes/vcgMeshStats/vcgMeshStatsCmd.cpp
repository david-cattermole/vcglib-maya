...
vcgMeshStats::vcgMeshStats()
//
//  Description:
//    vcgMeshStats constructor
//
{
//  m_percent = 100.0;
//  m_maxTime = 3600.0;
...
//
//  Description:
//    vcgMeshStats destructor
//
{}

void *vcgMeshStats::creator()
//
//  Description:
...
//
{
  return new vcgMeshStats();
}

bool vcgMeshStats::isUndoable() const
//
//  Description:
...
MStatus vcgMeshStats::doIt(const MArgList &args)
//
//  Description:
//    implements the MEL vcgMeshStats command.
//
//  Arguments:
...
//
{
  ...
  setModifierNodeType(vcgMeshStatsNode::id);
  ...
  if (found)
  {
    ...
    {
      setResult("vcgMeshStats command succeeded!");
    }
    else
    {
      displayError("vcgMeshStats command failed!");
    }
  }
  else
  {
    displayError("vcgMeshStats command failed: Unable to find selected components");
    status = MS::kFailure;
  }
...
MStatus vcgMeshStats::redoIt()
//
//  Description:
//    Implements redo for the MEL vcgMeshStats command.
//
...
MStatus vcgMeshStats::undoIt()
//
//  Description:
//    implements undo for the MEL vcgMeshStats command.
//
...
//
{
  ...
  if (status == MS::kSuccess)
  {
    setResult("vcgMeshStats undo succeeded!");
  }
  else
  {
    setResult("vcgMeshStats undo failed!");
  }

  return status;
}

MStatus vcgMeshStats::initModifierNode(MObject modifierNode)
{
  MStatus status;
...
MStatus vcgMeshStats::directModifier(MObject mesh)
{
  MStatus status;
