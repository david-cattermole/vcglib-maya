...
vcgWriteFile::vcgWriteFile()
//
//  Description:
//    vcgWriteFile constructor
//
{
//  m_percent = 100.0;
//  m_maxTime = 3600.0;
...
//
//  Description:
//    vcgWriteFile destructor
//
{}

void *vcgWriteFile::creator()
//
//  Description:
...
//
{
  return new vcgWriteFile();
}

bool vcgWriteFile::isUndoable() const
//
//  Description:
...
MStatus vcgWriteFile::doIt(const MArgList &args)
//
//  Description:
//    implements the MEL vcgWriteFile command.
//
//  Arguments:
...
//
{
  ...
  // Initialize the polyModifierCmd node type - mesh node alwritey set
  setModifierNodeType(vcgWriteFileNode::id);
  ...
  if (found)
  {
    ...
    {
      setResult("vcgWriteFile command succeeded!");
    }
    else
    {
      displayError("vcgWriteFile command failed!");
    }
  }
  else
  {
    displayError("vcgWriteFile command failed: Unable to find selected components");
    status = MS::kFailure;
  }
...
MStatus vcgWriteFile::redoIt()
//
//  Description:
//    Implements redo for the MEL vcgWriteFile command.
//
...
MStatus vcgWriteFile::undoIt()
//
//  Description:
//    implements undo for the MEL vcgWriteFile command.
//
...
//
{
  ...
  if (status == MS::kSuccess)
  {
    setResult("vcgWriteFile undo succeeded!");
  }
  else
  {
    setResult("vcgWriteFile undo failed!");
  }

  return status;
}

MStatus vcgWriteFile::initModifierNode(MObject modifierNode)
{
  MStatus status;
...
MStatus vcgWriteFile::directModifier(MObject mesh)
{
  MStatus status;
